...
def calculate_heading_change(waypoints, closest_waypoints, heading):
...
def reward_function(params):
    ...
    if is_steep_curve:
        ...
        SPEED_THRESHOLD = 2.5
        if speed < SPEED_THRESHOLD:
            reward += 2.2  # Higher reward for reducing speed at steep curve
        else:
            reward -= 1.5  # Higher penalty for going too fast and away from centre
        ...
        if distance_from_center < marker_2:
            reward += 2.5  # Bonus for maintaining optimal trajectory
    else:
        # General speed incentive on non-steep sections
        SPEED_THRESHOLD = 2.75
        if speed >= SPEED_THRESHOLD:
            reward += 2.5  # reward for maintaining high speed
